...
def save_model(model, version, epoch, save_type='loss'):
    save_path = os.path.join(f'./ckpts/{version}')
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    os.system(f'rm {os.path.join(save_path, f"best_{save_type}*")}')
    save_dir = os.path.join(save_path, f'best_{save_type}_{epoch}.pth')
...
def train_valid(epoch, model, trn_dl, val_dl, criterion, optimizer, scheduler, logger, device, beta=0.0, augmix_data=None, debug=True):
    ...
    with tqdm(trn_dl, total=len(trn_dl), unit='batch') as trn_bar:
        for batch, (images, masks, _) in enumerate(trn_bar):
            trn_bar.set_description(f"Train Epoch {epoch+1}")

            optimizer.zero_grad()
            # images, masks = sample['image'], sample['mask']
            # images, masks = torch.stack(sample['image']), torch.stack(sample['mask']).long()
            ...
            hist = add_hist(hist, masks.detach().cpu().numpy(), preds, n_class=num_cls)
    ...
    with torch.no_grad():
        with tqdm(val_dl, total=len(val_dl), unit='batch') as val_bar:
            for batch, (images, masks, image_infos) in enumerate(val_bar):
                val_bar.set_description(f"Valid Epoch {epoch+1}")

                images, masks = torch.stack(images), torch.stack(masks)
                images, masks = images.to(device), masks.to(device).long()
                # images, masks = torch.stack(sample['image']), torch.stack(sample['mask'])
                # images, masks = images.to(device), masks.to(device).long()
                ...
                input_np = np.array(Image.open(os.path.join('/opt/ml/segmentation/input/data', file_names[0])))
...
def train(epoch, model, trn_dl, pseudo_dl, criterion, optimizer, scheduler, logger, device, augmix_data=None):
    ...
    with tqdm(pseudo_dl, total=len(pseudo_dl), unit='batch') as pseudo_bar:
        for batch, (images, masks, _) in enumerate(pseudo_bar):
...
def valid(epoch, model, val_dl, criterion, logger, device, debug=True):
    ...
    with torch.no_grad():
        with tqdm(val_dl, total=len(val_dl), unit='batch') as val_bar:
            for batch, (images, masks, file_names) in enumerate(val_bar):
                val_bar.set_description(f"Valid Epoch {epoch+1}")
